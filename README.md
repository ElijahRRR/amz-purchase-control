...
| 替买家号切支付卡（所有者定稿①） | ⚠️ **只验到我们自己那一半**：期望卡随认领下发、「切完重读结算页」、切不动时不下单/不转人工/清车，都有 pytest 与 `--scenario card_switch / card_switch_fail` 实跑；`ensurePaymentCard` 那五步的**分支**（三种停法各停在哪一步、每一处超时先问登录态、第 ② 步「从无到有」那条对照判据、期望值非四位数字必须抛错、期望为空连 `doc()` 都不碰）有一组 DOM 断言拿真驱动跑在 `checkout.html` 上——那一段此前一条断言都没有，单测用的是假驱动、smoke 用的是模拟驱动、DOM 断言只覆盖三个纯函数。**仍然没人盯的**：向上爬找最小块那一段的文本判据、以及 ①「入口没找到」那一支。payselect 页那一整套**判据本身没有任何证据**——厂商自己承认那一页只有截图、没有 DOM 样本，夹具 `payselect.html` 是照他们的猜测造的。夹具上跑通只说明我们按自己写的语义在读。第一次开 live 档跑到这一步之前，先在真页面上核一遍 `selectors.checkout.payselect` |
...
而**这件事必须在库里留一行,不能只活在插件的内存里**:插件在点下单按钮之前
报一条 step 事件,服务端据此把 `tasks.may_have_ordered` 置 true(与追加事件同一事务),
重置回队列不清它。那道「先去买家号确认过再重置」的闸因此判**两条判据取或**:
错误码属于「可能已下单」那一组,**或者**这一单越过了下单点。
只判码会漏掉一整类 —— 越过下单点之后抛的 `DriverError` 用的是它自己的码
(`PLUGIN_INTERNAL` / `CART_MISMATCH`,都在「可重试」那一组),库里于是出现
「状态说要人裁决、码说重一下就过」的单,而它已经花过钱了。
**回队列的每一条路都判一次**:人工重置、批量重置、自动重试选单,
...
- 「可能已下单」那道二次确认闸曾按 `status` 判,而状态是**插件**在 `/fail` 里
  ...
  照样在喊「可能已经下单」,**喊完了静默重置**。改成按 `error_code` 判之后仍然漏一整类:
  越过下单点之后抛 `DriverError`,落库是「状态 manual + 码 PLUGIN_INTERNAL」——
  **两个字段各说各的,而闸只听其中一个**。现在库里有 `tasks.may_have_ordered`,
  记的是插件在点下单按钮之前自己说过的那句话。
  **这条教训还有第三段**:那道新判据先只装了三条路(人工重置 / 批量重置 /
  自动重试选单),而回到队列最直接的第四条 —— 插件自己调的 `/release` ——
  一行都没判。前三条至少还要人点一下,这一条连人都没有:插件那个
  `if (!mayHaveOrdered)` 改坏一次,任务就原地回 `ready` 并立刻被再次认领。
  整条演进读起来是:判 `status` → 判 `code` → 判 `code or 越过下单点` →
  **把这条判据装到每一条能回队列的路上**
