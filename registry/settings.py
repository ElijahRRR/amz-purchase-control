...
def price_consistency_tolerance_pct() -> int:
    """输入:无 → 输出:Σ单价×数量 与货款相差多少个百分点开始记一笔(默认 15)。

    **这是一条可观测性信号,不是护栏。** 超了不拦单,只在 guard_check 事件的
    载荷里留一句话。做成可调的理由是铁律 3:阈值散在业务代码里,想调的人
    找不到,找到了也不敢动。

    为什么默认 15 而不是更紧:结算页的商品面板单价是**税前、不含运费**的,
    而货款含运费和税(夹具那张单:2048.49 商品 + 12.99 运费 + 180.38 税 =
    2241.86,差 8.6%)。卡到 5% 会让几乎每一单都记一笔,而一条**每单都出现**
    的告警等于没有告警 —— 与 feishu_sync 那格「永远红着的卡片」同一个理由。

    0 和负数按 0 算,含义是「每一单都记」;真要关掉这条记录,
    把它设成一个大到不可能达到的数(比如 1000),而不是设成 0 ——
    0 看起来像「关掉」,实际是「全开」,这种反直觉的开关迟早坑人。
    """
    return max(0, int(os.environ.get("AMZ_PRICE_CONSISTENCY_TOLERANCE_PCT", "15")))
